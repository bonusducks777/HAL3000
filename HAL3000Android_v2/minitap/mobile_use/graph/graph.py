...
def post_contextor_gate(
    state: State,
) -> Literal["cortex", "planner"]:
    logger.info("Starting post_contextor_gate")
    # If no plan exists, go to planner first
    if not state.subgoal_plan:
        logger.info("No plan exists, routing to planner")
        return "planner"
    # If plan exists, go to cortex for execution
    logger.info("Plan exists, routing to cortex")
    return "cortex"
...
async def get_graph(ctx: MobileUseContext) -> CompiledStateGraph:
    ...
    # Linking nodes - FIXED: Strict sequential execution, no parallel paths
    ...
    graph_builder.add_edge("planner", "cortex")
    graph_builder.add_conditional_edges(
        "cortex",
        post_cortex_gate,
        {
